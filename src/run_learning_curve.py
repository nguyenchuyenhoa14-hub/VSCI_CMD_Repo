...
def run_unified_pipeline(cmd_tr_full, y_tr, cmd_vl_full, y_vl, lut, run_cv=True):
    ...
    def dpi(cmd, pt, pb):
        ...
        for i in range(N):
            ...
            # Correct boundary: Air-filled via (Bosch DRIE bare etched trench).
            # The via bottom interface is Air/Si substrate
            # (Bare-etched Air trench in Silicon substrate).
            # Silicon optical constants at UV-Vis (Palik/Green 1995)
            si_wls = np.array([240., 300., 350., 400., 450.])
            si_n   = np.array([ 3.5,  3.8,  4.1,  4.4,  4.3])
            si_k   = np.array([ 5.5,  2.5,  0.4,  0.1,  0.05])
            n_bottom = np.interp(wls, si_wls, si_n) + 1j * np.interp(wls, si_wls, si_k)
            r = (n_eff - n_bottom) / (n_eff + n_bottom)
            ...
            res = minimize_scalar(
                ...
                bounds=(2700., 3300.), method='bounded', options={'xatol':1e-4}
            )
            D[i] = res.x
        return D
    ...
    if run_cv == False:
        # 1. Fit on FULL data first to find the best hyperparameters
        gs_d.fit(Xt_b_final, y_tr[:,2]-pd_tr)
        best_model = gs_d.best_estimator_
        print(f"    Best params: {gs_d.best_params_}")
        
        N_total = len(Xt_b_final)
        N_sizes = [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, N_total]
        train_maes = []
        val_maes = []
        ...
        for n in N_sizes:
            print(f"    Training RBF Corrector with N={n}...")
            t_maes_n = []
            v_maes_n = []
            
            # Average over 5 random seeds to smooth out the curve
            for seed in range(5):
                np.random.seed(seed)
                idx = np.random.choice(N_total, n, replace=False)
                X_sub = Xt_b_final[idx]
                y_sub = yt_d[idx]
                
                best_model.fit(X_sub, y_sub)
                
                pt_sub = best_model.predict(X_sub)
                pv_all = best_model.predict(Xv_b_final)
                
                t_maes_n.append(np.mean(np.abs(y_sub - pt_sub)))
                v_maes_n.append(np.mean(np.abs(yv_d - pv_all)))
                
            train_maes.append(np.mean(t_maes_n))
            val_maes.append(np.mean(v_maes_n))
            
        np.savez('real_learning_curve.npz', N_sizes=N_sizes, train_mae=train_maes, val_mae=val_maes)
        data_save_path = os.path.join(DIR, 'real_learning_curve.npz')
        np.savez(data_save_path, N_sizes=N_sizes, train_mae=train_maes, val_mae=val_maes)
        print(f"    Saved smoothed real_learning_curve.npz to root and {data_save_path}!")
        sys.exit(0)
